...
export default async function CommandDraftReviewSurfacePage(props: PageProps) {
  ...
  let status: CommandDraftReviewStatus = "COMMAND_DRAFT_REVIEW_CONFIG_MISSING";
  ...
  const payload = {
    ...
    source: SOURCE,
    status,
    mode: "COMMAND_DRAFT_REVIEW_SURFACE_PAGE_READ_ONLY",
    incident_id: incidentId,
    workspace_id: workspaceId,
    dry_run: true,
    command_record_id: commandRecordId,
    command_status: commandRecordId ? "DRAFT_PERSISTED" : "NOT_FOUND",
    command_review: status === "COMMAND_DRAFT_REVIEW_READY" ? "READY" : "BLOCKED",
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-8 text-white sm:px-6 lg:px-8">
      <Card tone={status === "COMMAND_DRAFT_REVIEW_READY" ? "green" : "amber"}>
        <Eyebrow>Command Draft Review Surface</Eyebrow>

        <h1 className="max-w-4xl text-5xl font-black leading-[0.95] tracking-[-0.06em] sm:text-7xl">
          Review de Command Draft
        </h1>

        <p className="mt-8 max-w-3xl text-xl leading-10 text-white/60">
          V5.14 relit la Command Draft persistée et vérifie son état de sécurité.
          Cette surface est strictement read-only : aucune queue, aucun Run, aucun
          worker call, aucune mutation.
        </p>

        <div className="mt-8 flex flex-wrap gap-3">
          <Badge tone="neutral">{VERSION}</Badge>
          <Badge tone={status === "COMMAND_DRAFT_REVIEW_READY" ? "green" : "amber"}>
            {status}
          </Badge>
      ...
      <Card tone="green">
        ...
        <div className="grid gap-4">
          <DataTile label="Previous Version" value="Incident Detail V5.13" tone="green" />
          <DataTile label="Command Draft Persistence" value="VALIDATED" tone="green" />
          <DataTile label="Command Persisted As Draft" value={commandRecordId ?? "NOT_FOUND"} tone="green" />
          <DataTile label="No Queue / No Run / No Worker" value="CONFIRMED" tone="red" />
        </div>
      </Card>

      <Card tone="cyan">
        <Eyebrow>Command Draft Identity</Eyebrow>
      ...
      <Card tone="green">
        <Eyebrow>Command Safety State</Eyebrow>

        <div className="grid gap-4">
          <DataTile label="Status" value={commandStatus || "UNKNOWN"} tone={commandStatus === "Draft" ? "green" : "amber"} />
          ...
          <DataTile label="Dry Run" value={String(dryRun)} tone={dryRun === true ? "green" : "red"} />
          <DataTile label="Queue Allowed" value={String(queueAllowed)} tone={queueAllowed === true ? "red" : "green"} />
          ...
          <DataTile label="Real Run" value={realRun || "UNKNOWN"} tone={realRun === "Forbidden" ? "green" : "red"} />
          <DataTile label="Secret Exposure" value={secretExposure || "UNKNOWN"} tone={secretExposure === "Disabled" ? "green" : "red"} />
        </div>
      </Card>

      <Card tone="cyan">
        <Eyebrow>Command Input JSON</Eyebrow>
        <JsonBlock value={commandInputJson} />
      </Card>

      <Card tone="green">
        <Eyebrow>Review Checklist</Eyebrow>
      ...
      <Card tone="red">
        ...
        >
          Queue future non activée
        </button>
      </Card>

      <Card>
        <Eyebrow>Read-only Command Review Payload</Eyebrow>
