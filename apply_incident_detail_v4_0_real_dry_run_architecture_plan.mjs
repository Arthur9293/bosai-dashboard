...
const filePath = path.join(
  docsDir,
  "incident-detail-v4-0-real-dry-run-architecture-plan.md"
);

const marker = "Incident Detail V4.0 — Real Dry Run Architecture Plan";
...
if (fs.existsSync(filePath)) {
  ...
  if (existing.includes(marker)) {
    console.log("V4.0 architecture plan déjà présent. Aucune modification.");
    process.exit(0);
  }

  console.error(
    "Un fichier docs/incident-detail-v4-0-real-dry-run-architecture-plan.md existe déjà mais ne contient pas le marker V4.0."
  );
  console.error("Patch arrêté pour éviter d’écraser un document existant.");
  process.exit(1);
}
...
console.log("V4.0 architecture plan créé avec succès.");
console.log(`Fichier créé : ${filePath}`);
console.log("Aucun fichier applicatif modifié.");
console.log("Aucun worker, aucun endpoint, aucune mutation, aucun POST /run.");
