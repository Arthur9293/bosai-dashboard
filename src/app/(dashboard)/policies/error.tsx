...
export default function Error({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    console.error("Policies segment error:", error);
  }, [error]);

  return (
    <div className="space-y-8">
      <DashboardCard
        title="Surface Policies indisponible"
        subtitle="Le segment Policies a rencontré une erreur, mais le cockpit reste protégé."
      >
        <div className="space-y-5">
          <div className="rounded-[20px] border border-red-500/20 bg-red-500/10 px-4 py-4">
            ...
            <div className="mt-2 text-sm leading-6 text-red-200">
              {error?.message || "policies_segment_error"}
            </div>
            {error?.digest ? (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <Link href="/tools" className={actionButtonClassName("default")}>
              Ouvrir Tools
            </Link>
          </div>
          ...
          <div className="rounded-[18px] border border-white/10 bg-black/20 px-4 py-4">
            ...
            <div className="mt-2 text-sm leading-6 text-zinc-300">
              Cette erreur est isolée à la surface Policies. Le shell BOSAI peut
