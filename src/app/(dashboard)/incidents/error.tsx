...
export default function IncidentsError({
  ...
  const safeMessage = useMemo(() => {
    ...
    if (!raw) return "La surface Incidents a rencontré une erreur inattendue.";
    return raw;
  }, [error]);
  ...
  return (
    <ControlPlaneShell
      ...
      title="Incidents"
      description="Erreur isolée sur la surface Incidents. Le shell global et la navigation restent préservés."
      badges={[
        ...
        <>
          <SidePanelCard title="Diagnostic">
            <div className="space-y-3 text-sm leading-6 text-white/70">
              <p>
                L’erreur est contenue au segment <span className="text-white/90">Incidents</span>.
              </p>
              <p>
          ...
          <SidePanelCard title="Action">
            <div className="space-y-3 text-sm leading-6 text-white/70">
              ...
              <p>
                Si l’erreur persiste, vérifier la lecture API de la page incidents ou du helper associé.
              </p>
            </div>
          </SidePanelCard>
        </>
    ...
    >
      <SectionCard
        title="Erreur surface Incidents"
