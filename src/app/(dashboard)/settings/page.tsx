...
async function fetchWorkspaceUsage(params?: {
  ...
  try {
    ...
    const data = (await response.json()) as WorkspaceUsageResponse;
    return data;
  } catch (error) {
    return {
      ok: false,
      error: error instanceof Error ? error.message : "workspace_usage_fetch_failed",
    };
  }
}
...
function WarningList({
  ...
  return (
    <DashboardCard
      title="Quota signals"
      subtitle="Warnings soft et état de blocage hard."
    >
      <div className="space-y-3">
        {blocked ? (
          <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-200">
            Blocked: {blockReason || "quota_blocked"}
          </div>
        ) : null}
...
export default async function SettingsPage() {
  ...
  const envReady =
    Boolean(process.env.BOSAI_WORKER_URL || process.env.NEXT_PUBLIC_BOSAI_WORKER_URL) &&
        ...
        <>
          <section className="space-y-4 border-b border-white/10 pb-6">
            ...
            <div className="space-y-4 xl:flex xl:items-end xl:justify-between xl:gap-8 xl:space-y-0">
              ...
              <div className="flex flex-wrap gap-2">
                <span className={badgeClassName(statusBadgeVariant(workspace?.is_active, blocked))}>
                  {blocked ? "BLOCKED" : workspace?.is_active ? "ACTIVE" : "INACTIVE"}
                </span>

                {workspace?.type ? (
                  <span className={badgeClassName("info")}>{workspace.type.toUpperCase()}</span>
                ) : null}

                {workspace?.slug ? (
                  <span className={badgeClassName("default")}>{workspace.slug}</span>
                ) : null}
              </div>
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 gap-4 xl:grid-cols-3">
            ...
            <DashboardCard
              ...
              subtitle="Informations de base du workspace SaaS."
            >
              <div className="space-y-3 text-sm text-zinc-400">
                <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center justify-between gap-4">
                  <span>Plan ID</span>
                  <span className="text-zinc-200">{workspace?.plan_id || "—"}</span>
                </div>
          ...
          <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            ...
            >
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 text-sm text-zinc-400">
            ...
            >
              <div className="space-y-3 text-sm text-zinc-400">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span
                    className={
                      previewBlocked ? "text-red-300 font-medium" : "text-emerald-300 font-medium"
                    }
                  >
